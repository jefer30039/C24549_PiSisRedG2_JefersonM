...
public:
  ...
  virtual const char *GetCipher() { return "None"; }

  int Bind(int); // Assign a socket address to a socket descriptor
  int MarkPassive(
      int); // Mark a socket passive: will be used to accept connections
  int WaitForConnection(void); // Wait for a peer connection
  virtual VSocket *AcceptConnection() = 0;
  int Shutdown(
      int); // cause all or part of a full-duplex connection on the socket
