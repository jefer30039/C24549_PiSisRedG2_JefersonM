...
struct EntradaDirectorio {
    string nombre;      // e.g. "servidor-figuras", "intermediario-1"
    string rol;         // "SERVIDOR" | "INTERMEDIARIO"
    bool activo;
};
...
bool servidor_descubierto   = false;   // intermediario encontró al servidor
...
// Devuelve true si encontró una entrada con el rol buscado, y escribe el nombre en 'encontrado'
...
// ============================================================
//  PROTOCOLO DE DESCUBRIMIENTO  (mensajes ANNOUNCE / DISCOVER / ACK_DISCOVER)
...
// ============================================================
//  HILO DE DESCUBRIMIENTO – INTERMEDIARIO
...
void* discovery_intermediario(void* arg) {
    ...
    // 1. Anunciarse en el directorio (ANNOUNCE)
    ...
    // Publicar mensaje ANNOUNCE en el buzón de discovery para que otros lo vean
    {
        Mensaje anuncio;
    ...
    // 2. Buscar al servidor de Figuras (DISCOVER) con reintentos
    int intentos = 0;
    while (!terminado && !servidor_descubierto) {
        ...
        if (buscarEnDirectorio(ROL_BUSCADO, nombreServidor)) {
            ...
            usleep(500000); // espera 0.5 s antes de reintentar
        }
    }
...
// ============================================================
//  HILO DE DESCUBRIMIENTO – SERVIDOR DE FIGURAS
...
void* discovery_servidor(void* arg) {
    ...
    // Pequeña pausa para que el intermediario arranque primero (simula latencia de red)
    usleep(200000);
    ...
    // 2. Buscar al intermediario (DISCOVER) con reintentos
    int intentos = 0;
...
int main() {
    ...
    // ---- hilos de descubrimiento (arrancan primero) ----
    pthread_create(&hilos[4], NULL, discovery_intermediario, NULL);
    pthread_create(&hilos[5], NULL, discovery_servidor, NULL);

    // Esperamos a que ambos lados se hayan encontrado antes de arrancar
    // el flujo principal (máx ~3 s)
    {
        int espera = 0;
